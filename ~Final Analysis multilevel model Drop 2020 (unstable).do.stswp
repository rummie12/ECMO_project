**Into GitBash**
cd "C:\Users\laxfr\Desktop\All-Comers\Analysis"
git init "C:\Users\laxfr\Desktop\All-Comers\Analysis\ECMO1"
...
**Generating ECMO dx ICD categories**
gen ECMO_icd10 = substr(ECMO_dx_ICD, 1, 2)
encode ECMO_icd10, gen(ECMO_icd10_cat)


**Generating ECMO Indication tables**
*all patients*
preserve
...
*2024*
bysort obsnum: egen total_admissions_2024_nhr = count(DischargeID) if high_risk_group ==3 & YearOfService ==9
...
*2024*
bysort obsnum: egen total_admissions_2024_T1318 = count(DischargeID) if (high_risk_group ==0 | high_risk_group == 4 | high_risk_group ==5) & YearOfService ==9 
...
*2024*
bysort obsnum: egen total_admissions_2024_trauma = count(DischargeID) if (high_risk_group ==1 | high_risk_group == 4 | high_risk_group ==6) & YearOfService ==9
...
*2024*
bysort obsnum: egen total_admissions_2024_onc = count(DischargeID) if (high_risk_group ==2 | high_risk_group == 5 | high_risk_group ==6) & YearOfService ==9
...
replace center_admissions_t1318 =0 if center_admissions_t1318 ==.
...
*Assigning values to all patients*
bysort HospitalNumber YearOfService(center_admissions_t1318): replace center_admissions_t1318 = center_admissions_t1318[_N]
...
replace center_admissions_trauma = 0 if center_admissions_trauma ==.
...
*assigning values to all patients*
bysort HospitalNumber YearOfService(center_admissions_trauma): replace center_admissions_trauma = center_admissions_trauma[_N]
...
replace center_admissions_onc = 0 if center_admissions_onc ==.
...
*assigning values to all patients*
bysort HospitalNumber YearOfService(center_admissions_onc): replace center_admissions_onc = center_admissions_onc[_N]
...
twoway ///
	...
	yscale(log) ///
    legend(order(1 "Total ECLS (Mean = 1,222)" 2 "Total Admissions (Mean = 109,698)"))
restore
~pause
...
twoway ///
	...
	xlabel(1 "2016" 2 "2017" 3 "2018" 4 "2019" ///
           6 "2021" 7 "2022" 8 "2023" 9 "2024", angle(-45)) ///
    ytitle("Number of Cases") ///
    legend(order(1 "Total Admissions (Mean = 418)"))
...
twoway ///
    ...
    xlabel(1 "2016" 2 "2017" 3 "2018" 4 "2019" 5 "2020" ///
           6 "2021" 7 "2022" 8 "2023" 9 "2024", angle(-45)) ///
    ytitle("Number of Cases") ///
    legend(order(1 "Total ECMO (Mean = 2)" 2 "Total Admissions (Mean = 405)"))
restore
...
twoway ///
    ...
    xlabel(1 "2016" 2 "2017" 3 "2018" 4 "2019" 5 "2020" ///
           6 "2021" 7 "2022" 8 "2023" 9 "2024", angle(-45)) ///
    ytitle("Number of Cases") ///
    legend(order(1 "Total ECMO (Mean = 13)" 2 "Total Admissions (Mean = 3,923)"))
restore
...
twoway ///
    ...
    ytitle("Number of Cases") ///
	yscale(log) ///
    legend(order(1 "Total ECMO (Mean = 1,026)" 2 "Total Admissions (Mean = 94,816)"))
restore
...
codebook center_admissions_by_year
codebook center_annual_total_ecmo
codebook center_admissions_onc
codebook center_admissions_t1318
codebook center_admissions_trauma
...
*High Risk Groups*
*T13/18 probability*
di .0094521*.4340937/(.0094521*.4340937  + 1) * 100
*Trauma probability*
di .0094521*2.150589/(.0094521*2.150589   + 1) * 100
*Onc probability*
di .0094521*.3528841 /(.0094521*.3528841  + 1) * 100
...
*baseline*
*lower*
di .0081158/(.0081158+1)*100
*upper*
di .0110084/(.0110084+1)*100
*T1318 CI*
*lower*
di .0094521*.2680969/(.0094521*.2652378+1) * 100
*upper*
di .0094521*.7178255/(.0094521*.7104467+1) * 100

*Trauma CI*
*lower*
di .0094521*2.033026/(.0094521*2.020166+1) * 100
*upper*
di .0094521*2.303532/(.0094521*2.289432+1) * 100

*Onc/HSCT*
*lower*
di .0094521*.2878782/(.0094521*.2882862+1) * 100
*upper*
di .0094521*.4312882/(.0094521*.4319568+1) * 100
...
table1, by(ECMO_y_n) vars(admitageyears contn \ Ethnicity cat \ Gender cat \ high_risk_group cat Gender cat \ Mortality cat)
...
*Ethnicity*
*White*
di .0094521*1.078656  /(.0094521*1.078656  + 1) * 100


*Gender*

*Female*
di .0094521* 1.151565/(.0094521* 1.151565 + 1) * 100

~pause
*Age*
di .0094521*.950508 /(.0094521*.950508  + 1) * 100

*Year of Service*
di .0094521*1.021202 /(.0094521*1.006275   + 1) * 100
...
*Ethnicity*
*White*
di .0094521* 1.019961   /(.0094521* 1.019961   + 1) * 100
*upper*
di .0094521*1.14073 /(.0094521*1.14073 + 1) * 100
~pause

*Gender*
*Female*
*lower*
di .0094521*1.104197 /(.0094521*1.104197+ 1) * 100
*upper*
...
**onc**
di .0094521* 1.000498/(.0094521*1.000498+ 1) * 100

**t13/18**
di .0094521* .9977184/(.0094521*.9977184+ 1) * 100

**trauma**
di .0094521* 1.000911/(.0094521*1.000911+ 1) * 100
...
*onc*
*lower*
di .0094521* .9995351/(.0094521*.9995351+1) * 100
*upper*
di .0094521*1.001463/(.0094521*1.001463+1) * 100

*t13/18*
*lower*
di .0094521* .9931351 /(.0094521*.9931351 +1) * 100
*upper*
di .0094521*1.002323/(.0094521*1.002323+1) * 100
